...
const AUMPAD=new RegExp(/(AumPad)/);
...
function assign_api()
{
	...
	post("Done building Ohm64 API Objects.\n");
}
...
{	
	...
	if (alive > 0)
	{
		switch(messagename)
		{
			...
			//	post('page1');
			//	page_str = '1';
			//	monobridge.call('osc_in', messagename, 'None')
			//	break;
			//case "/2":
			//	post('page2');
			//	page_str = '2';
			...
			case "/2/xy_0":
				...
				messnamed('aumpad_xy', 1, args[2] * 100);
				//post('x', args[0], 'y', args[1], '\n');
				break;
			case "/2/xy_1":
				messnamed('aumpad_xy', 2, args[1] * 100);
				messnamed('aumpad_xy', 3, args[2] * 100);
				//post('x', args[1], 'y', args[2], '\n');
				break;
			default:
