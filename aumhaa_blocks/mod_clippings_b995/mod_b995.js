...
var absolute_mode = 1;
var c_local_ring_control = 1;
var c_absolute_mode = 1;
...
var stored_c_keys = [];						 //array to store key values received when the script is either dead or disconnected
var k=31;do{
	stored_c_keys[k]=0;
}while(k--);
var stored_rings = [];
...
function init()
{
	if(DEBUG||DEBUGCNX){post('init_b995\n');}
...
function send_stored_data()
{
	...
	surface.call('set_c_local_ring_control', c_local_ring_control);		   //set up local ring control for the CS Script
	surface.call('set_c_absolute_mode', c_absolute_mode);						//set up whether the encoders should send relative or absolute values
	...
	var x=3;do{
		var y=3;do{
			_c_grid(x, y, stored_c_grid[x][y]);
		}while(y--);
	}while(x--);
	var k=31;do{
		_c_key(k, stored_c_keys[k]);
	}while(k--);
	var r=3;do{
		var t=2;do{
	...
	if((args[0]=='value')&&(args[1]!='bang'))
	{
		switch(args[1])
		{
			...
			/*case('knob'):
				...
				break;*/
			case('c_grid'):
				messnamed(unique + 'c_grid', args[2], args[3], args[4]);
				outlet(4, 'c_grid', args[2], args[3], args[4]);
				break;
			case('c_key'):
				messnamed(unique + 'c_key', args[2], args[3], args[4]);
				outlet(4, 'c_key', args[2], args[3]);
				break;
			case('c_dial'):
				messnamed(unique + 'c_wheel', 'dial', args[2], args[3], args[4]);
				outlet(4, 'c_dial', args[2], args[3], args[4]);
				break;
			case('c_dial_button'):
				messnamed(unique + 'c_wheel', 'button', args[2], args[3], args[4]);
				outlet(4, 'c_button', args[2], args[3], args[4]);
				break;
			case('dial'):
			...
			case('hotline'):
				...
				break;
			case 'pop':
				messnamed(unique+'pop', popmess[args[2]]);
				break;
			default:
...
function Anything()
{
	...
	switch(args[0])
	{
		...
		case 'key':
			...
			break;
		case 'c_grid':
			stored_c_grid[args[1]][args[2]] = args[3];
			break;
		case 'c_key':
			stored_c_keys[args[1]] = args[2];
			break;
		case 'button':
		...
		case 'set_color_map':
			...
			make_maps(color_maps);
			break;
		case 'set_local_ring_control':
			local_ring_control = args[1];
			break;
		case 'set_absolute_mode':
			absolute_mode = args[1];
			break;
		case 'set_c_local_ring_control':
			c_local_ring_control = args[1];
			break;
		case 'set_c_absolute_mode':
			c_absolute_mode = args[1];
			break;
		default:
			break;
	}
...
function _anything()
{
	...
	switch(args[0])
	{
		...
		case 'c_wheel':
			...
			if(args[3] == 'custom')
			{
				to_c_wheel(args[1], args[2], args[3], 'x' + args.slice(4, -1).join(''));
			}
			else
			{	  
				if(args.length==5)
				{	  
					to_c_wheel(args[1], args[2], args[3], args[4]);
				}
			}
			break;
		case 'column':
...
//call from the patch 4x4 c_grid, to be sent to the controller
function _c_grid(x, y, val)
{
	surface.call('receive_c_grid', x, y, val);
	stored_c_grid[x][y] = val;
}

//call from the patch to the 32 c_keys, addressed as 0-31
function _c_key(num, val)
{
	surface.call('receive_c_key', num, val);
	stored_c_keys[num] = val;
}
...
{
	to_c_wheel(x, y, 'white', val);
	stored_buttons[x][y]=val;
}

//call from the patch to the 4x3 encoder rings
function _c_ring(x, y, val)
{
	to_c_wheel(x, y, 'value', val);
	stored_rings[x][y]=val;
}
...
//call from patch to the 4x3 endless encoders
function _to_c_wheel(x, y, parameter, value)
{
	if(DEBUG){post('_to_c_wheel', x, y, parameter, value, '\n');}
	surface.call('receive_c_wheel', x + (y * 4), parameter, value);	 
}
...
function _mask()
{
	...
	switch(args[0])
	{
		...
		case 'row':
			...
			break;
		case 'c_key':
			surface.call('receive_mask_c_key', args[1], args[2]);
			break;
		case 'c_grid':
			surface.call('receive_mask_c_grid', args[1], args[2], args[3]);
			break;
		case 'c_column':
			surface.call('receive_mask_c_column', args[1], args[2]);
			break;
		case 'c_row':
			surface.call('receive_mask_c_row', args[1], args[2]);
			break;
	}
}
...
function _batch()
{
	...
	switch(args[0])
	{
		...
		case 'row':
			...
			break;
		case 'c_grid':
			surface.call('receive_c_grid_all', args[1]);
			var i=3;do{
				stored_c_grid[i]=[];
				var j=3;do{
					stored_grid[i][j] = args[1];
			...
			break;
		case 'c_column':
			surface.call('receive_c_grid_column', args[1], args[2]);
			var i=3;do{
				stored_c_grid[args[1]][i] = args[2];
			}while(i--);
			break;
		case 'c_row':
			surface.call('receive_c_grid_row', args[1], args[2]);
			var i=3;do{
				stored_c_grid[i][args[1]] = args[2];
			}while(i--);
			break;
	}
}
...
//set the number of parameters used by the MonoDeviceComponent...if not set, it will default to the 
//number of parameter controls used by the Monomodular compatible scripts that are connected.
function _set_number_params(val)
{
	surface.call('receive_device', 'set_number_params', val);
}

function _set_number_custom(val)
{
	surface.call('receive_device', 'set_number_custom', val);
}

function _set_custom_parameter(num, id)
{
	surface.call('receive_device', 'set_custom_parameter', num, 'id', id);
}

function _set_custom_parameter_value(num, val)
{
	surface.call('receive_device', 'set_custom_parameter_value', num, val);
}
...
//turn on/off local ring control for the selected modClient
function _set_c_local_ring_control(val)
{
	surface.call('set_c_local_ring_control', val);
}

//turn on/off absolute mode for the encoders of the selected modClient
function _set_c_absolute_mode(val)
{
	surface.call('set_c_absolute_mode', val);
}
...
function _send_midi(type, num, val)
{
	//var args = arrayfromargs(arguments);
	//surface.call('send_midi', args.join('^'));
	if(DEBUG){post('send midi', type, num, val, '\n');}
	surface.call('send_midi', type, num, val);
}
