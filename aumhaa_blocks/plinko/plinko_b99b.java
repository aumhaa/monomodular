					...
					for(int nn=0;nn<4;nn++)			///interpret whether potential blooms from program data and randomizer	 !!!!!!turn into switch
					{
						...
						{  
							...
							{					   
								...
								{	
									thru=0;									///destroy the old particle
									//bin = dir.toString(2);			  ///cast the dump value to a binary string, split to array, and reverse
									//int new_dir = dir +256;
									String bin = Integer.toBinaryString(dir + 256);
									//post("bin " + bin);
									//bin_split = bin.split("").reverse();	  ///reverse the order of the array so that the first bit is "english first"
									char[] bin_split = bin.toCharArray();
									//post("bin_split" + bin_split);  ///this has to be formatted into an 8 position array
									for(int cd=1;cd<9;cd++)			///recurse for length of binary array
									{	
										//post("if bin_split "  + bin_split[cd]);
										if(bin_split[cd]=='1')//Integer.parseInt(bin_split[cd])>0)		/// if a value of a binary position of the array is non-zero, create a potential 
										{									///	   with the direction value of the position in the binary array.
											set_potential(xx,yy,cd);		///place potential in the temp_array for updating on next turn
										}
									}
								}
							}
