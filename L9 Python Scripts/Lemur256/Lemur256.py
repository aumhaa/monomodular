...
class Lemur256(ControlSurface):
	...
	def __init__(self, *a, **k):
		...
		with self.component_guard():
			self.log_message('submatrix:' + str(self._monomod256.submatrix[:, :]))
...
class LemurModHandler(ModHandler):
	...
	def update(self, *a, **k):
		mod = self.active_mod()
		self.log_message('update1')
		with self._script.component_guard():
			if not mod is None:
				self.log_message('update2')
				if self._grid:
				...
				self.log_message('before restore...')
				mod.restore()
				self.log_message('after restore...')
			else:
				if not self._grid_value.subject is None:
					self._grid_value.subject.reset()
				if not self._keys_value.subject is None:
					self._keys_value.subject.reset()
			self.log_message('update3')
			if self.is_shifted():
				self.log_message('updateagain, shifted')
				self._grid and self._device_selector.set_matrix(self._grid.submatrix[:, :1])
			else:
				self.log_message('updateagain, not shifted')
				self._device_selector.set_matrix(None)
			self.log_message('update finished__________________________')
