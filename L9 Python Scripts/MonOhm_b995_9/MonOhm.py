...
class MonOhm(ControlSurface):
	...
	def _assign_session_colors(self):
		num_tracks = 4
		num_scenes = 5 
		self._session.set_stop_track_clip_value(self._color_defs['STOP_ALL'])
		self._session2.set_stop_track_clip_value(self._color_defs['STOP_ALL'])
		self._session_main.set_stop_track_clip_value(self._color_defs['STOP_ALL'])
		for row in range(num_scenes): 
			for column in range(num_tracks):
				self._scene[row].clip_slot(column).set_triggered_to_play_value(self._color_defs['CLIP_TRG_PLAY'])
				self._scene[row].clip_slot(column).set_triggered_to_record_value(self._color_defs['CLIP_TRG_REC'])
				self._scene[row].clip_slot(column).set_stopped_value(self._color_defs['CLIP_STOP'])
				self._scene[row].clip_slot(column).set_started_value(self._color_defs['CLIP_STARTED'])
				self._scene[row].clip_slot(column).set_recording_value(self._color_defs['CLIP_RECORDING'])	
				self._scene2[row].clip_slot(column).set_triggered_to_play_value(self._color_defs['CLIP_TRG_PLAY'])
				self._scene2[row].clip_slot(column).set_triggered_to_record_value(self._color_defs['CLIP_TRG_REC'])
				self._scene2[row].clip_slot(column).set_stopped_value(self._color_defs['CLIP_STOP'])
				self._scene2[row].clip_slot(column).set_started_value(self._color_defs['CLIP_STARTED'])
				self._scene2[row].clip_slot(column).set_recording_value(self._color_defs['CLIP_RECORDING'])	
		for row in range(num_scenes): 
			for column in range(8):
				self._scene_main[row].clip_slot(column).set_triggered_to_play_value(self._color_defs['CLIP_TRG_PLAY'])
				self._scene_main[row].clip_slot(column).set_triggered_to_record_value(self._color_defs['CLIP_TRG_REC'])
				self._scene_main[row].clip_slot(column).set_stopped_value(self._color_defs['CLIP_STOP'])
				self._scene_main[row].clip_slot(column).set_started_value(self._color_defs['CLIP_STARTED'])
				self._scene_main[row].clip_slot(column).set_recording_value(self._color_defs['CLIP_RECORDING'])		
		self._session_zoom.set_stopped_value(self._color_defs['ZOOM_STOPPED'])
		self._session_zoom.set_playing_value(self._color_defs['ZOOM_PLAYING'])
		self._session_zoom.set_selected_value(self._color_defs['ZOOM_SELECTED'])
		self._session_zoom2.set_stopped_value(self._color_defs['ZOOM_STOPPED'])
	...
	def zoom_off(self):
		for column in range(4):
			self._grid[column][5].set_on_value(self._color_defs['MUTE'])
			self._mixer.channel_strip(column).set_mute_button(self._grid[column][5])
			self._grid[column][6].set_on_value(self._color_defs['SOLO'])
			self._mixer.channel_strip(column).set_solo_button(self._grid[column][6])
			self._grid[column][7].set_on_value(self._color_defs['ARM'])
			...
			if(self._r_function_mode._mode_index in range(0,3)):
				self._grid[column + 4][5].set_on_value(self._color_defs['MUTE'])
				self._mixer2.channel_strip(column).set_mute_button(self._grid[column + 4][5])
				self._grid[column + 4][6].set_on_value(self._color_defs['SOLO'])
				self._mixer2.channel_strip(column).set_solo_button(self._grid[column + 4][6])
				self._grid[column + 4][7].set_on_value(self._color_defs['ARM'])
				...
				for row in range(5):
					self._grid[column + 4][row].send_value(self._color_defs['USER1_COLOR'], True)
	...
	def zoom_left(self):
		...
		self._grid[1][5].set_on_value(self._color_defs['OVERDUB'])
		self._transport.set_overdub_button(self._grid[1][5])
		self._grid[2][5].set_on_value(self._color_defs['LOOP'])
		self._transport.set_loop_button(self._grid[2][5])
		self._grid[3][5].set_on_value(self._color_defs['STOP_ALL'])	
	...
	def assign_device_nav_to_menu(self):
		self._menu[2].set_on_value(self._color_defs['DEVICE_LOCK'])
		self._device.set_lock_button(self._menu[2])
		self._menu[1].set_on_value(self._color_defs['DEVICE_ON'])
		self._device.set_on_off_button(self._menu[1])
		for index in range(2):
			self._menu[index + 4].set_on_value(self._color_defs['DEVICE_NAV'])
			self._menu[index * 3].set_on_value(self._color_defs['DEVICE_BANK'])
	...
	def assign_transport_to_menu(self):
		self._menu[0].set_on_off_values(self._color_defs['PLAY_ON'], self._color_defs['PLAY'])
		self._transport.set_play_button(self._menu[0])	
		self._menu[2].set_on_value(self._color_defs['RECORD['])
		self._transport.set_record_button(self._menu[2])
		self._menu[1].set_on_value(self._color_defs['STOP'])
		self._transport.set_stop_button(self._menu[1])
		self._menu[3].set_on_value(self._color_defs['LOOP'])
		self._transport.set_loop_button(self._menu[3])	
		self._menu[5].set_on_value(self._color_defs['OVERDUB'])
		self._transport.set_overdub_button(self._menu[5])
		self._menu[4].set_on_value(self._color_defs['STOP_ALL'])
		self._session.set_stop_all_clips_button(self._menu[4])
	

	def assign_session_nav_to_menu(self):
		self._menu[1].set_on_off_values(self._color_defs['PLAY_ON'], self._color_defs['PLAY'])
		self._transport.set_play_button(self._menu[1])	
		self._menu[2].set_on_off_values(self._color_defs['STOP'], self._color_defs['STOP'])	
		self._transport.set_stop_button(self._menu[2])
		for index in range(2):
			self._menu[index + 4].set_on_value(self._color_defs['SESSION_NAV'])
			self._menu[index * 3].set_on_value(self._color_defs['SESSION_NAV'])
	...
	def assign_session_bank_to_menu(self):
		...
		self._transport.set_stop_button(self._menu[1])
		for index in range(2):
			self._menu[index + 4].set_on_value(self._color_defs['BANK_BUTTONS'])
			self._menu[index * 3].set_on_value(self._color_defs['BANK_BUTTONS'])
		self._session.set_track_bank_buttons(self._menu[5], self._menu[4])
		self._session.set_scene_bank_buttons(self._menu[3], self._menu[0])
