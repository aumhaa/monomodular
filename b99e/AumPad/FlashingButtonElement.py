...
all_colors=["white", "yellow", "orange", "blue", "gray", "green", "red", "purple"];
...
class FlashingButtonElement(ButtonElement):
	...
	def __init__(self, is_momentary, msg_type, channel, identifier, name, osc, osc_alt, cs):
...
#			self._is_being_forwarded = self._install_forwarding(self)
	...
	def send_midi(self, message):
		...
		if color < 1:
			...
			self._script._monobridge._send_osc(str(self.osc_alt+'/color'), all_colors[color])
