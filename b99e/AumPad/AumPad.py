...
class AumPad(ControlSurface):
	...
	def __init__(self, c_instance):
		...
		self.log_message(time.strftime("%d.%m.%Y %H:%M:%S", time.localtime()) + "--------------= AumPad log opened =--------------") 
	...
	def _setup_controls(self):
		...
		for index in range(8):
			self._button[index] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, OHM_BUTTONS[index], 'Button_' + str(index), '/2/p_button/'+str(index), '/2/c_button/'+str(index),  self)
		...
		for index in range(6):
			self._menu[index] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, OHM_MENU[index], 'Menu_' + str(index), '/2/p_menu/'+str(index), '/2/c_menu/'+str(index), self)	
		...
		self._livid = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, LIVID, 'Livid_Button', '/2/p_livid', '/2/c_livid', self)
		self._shift_l = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, SHIFT_L, 'Shift_Button_Left', '/2/p_shift_l', '/2/c_shift_l', self)
		self._shift_r = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, SHIFT_R, 'Shift_Button_Right', '/2/p_shift_r', '/2/c_shift_r', self)
		...
		for column in range(8):
			...
			for row in range(8):
				self._grid[column][row] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, (column * 8) + row, 'Grid_' + str(column) + '_' + str(row), '/2/p_grid/'+str(column)+'/'+str(row), '/2/c_grid/'+str(column)+'/'+str(row), self)
		for row in range(5):
			button_row = []
		...
		for column in range(16):
			...
			for row in range(16):
				self._square[column][row] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, int(column/8) + 1, row + ((column%8) * 16), '256Grid_' + str(column) + '_' + str(row), '/1/p_grid/'+str(column)+'/'+str(row), '/1/c_grid/'+str(column)+'/'+str(row), self)
		...
		for index in range(8):
			self._bank_button[index] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, 15, index, '256Grid_Bank_' + str(index), '/1/p_a/'+str(index), '/1/c_a/'+str(index), self)
		button_row = []
		for index in range(8):
		...
		for index in range(8):
			self._key_button[index] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, 15, index+8, '256Grid_Key_' + str(index), '/1/p_s/'+str(index), '/1/c_s/'+str(index), self)
		for index in range(8):
	...
	def _setup_monomod(self):
		...
		#self._host2._set_key_buttons(self._key_buttons)
		#self._host2._set_bank_buttons(self._bank_buttons)
		#self._host2._set_menu_buttons(self._script._menu)
		#self._host2.set_enabled(True)
		self.hosts = [self._host, self._host2]
		#self.show_message('256 Monomod grid enabled')
	...
	def _setup_touchosc(self):
		self._osc_registry = {}
		self._osc_registry['/ping'] = self._monobridge.osc_extra
		self._osc_registry['/1'] = self._monobridge.osc_extra
		self._osc_registry['/2'] = self._monobridge.osc_extra
