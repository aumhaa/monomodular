...
class BlockMod(ControlSurface):
	...
	def __init__(self, c_instance):
		...
		self.log_message("--------------= BlockMod log opened =--------------") 
		self.set_suppress_rebuild_requests(True)
		self._monomod_version = 'b992'
		self._codec_version = 'b992'
		...
		self.show_message('BlockMod Control Surface Loaded')
	...
	def _setup_controls(self):
		...
		for index in range(2):
			self._fader[index] = MonoEncoderElement2(MIDI_CC_TYPE, CHANNEL, SLIDER_CC[index], Live.MidiMap.MapMode.absolute, 'Fader_' + str(index), index, self)
		...
		for index in range(8):
			self._dial[index] = MonoEncoderElement2(MIDI_CC_TYPE, CHANNEL, KNOB_CC[index], Live.MidiMap.MapMode.absolute, 'Dial_' + str(index), index + 8, self)
		for index in range(4):
			self._menu[index] = FlashingButtonElement(is_momentary, MIDI_NOTE_TYPE, CHANNEL, FUNCTION_NOTES[index], 'Menu_' + str(index), self)	
	...
	def zoom_off(self):
		for column in range(4):
			...
			self._grid[column][6]._on_value = ARM[self._rgb]
			for row in range(5):
			...
			if(self._r_function_mode._mode_index in range(0,3)):
				...
				self._grid[column + 4][6]._on_value = ARM[self._rgb]
	...
	def zoom_left(self):
		...
		for index in range(4):
			...
			self._grid[index][6]._off_value = TRACK_STOP[self._rgb]
			track_stop_buttons.append(self._grid[index][6])
			self._grid[index + 4][6]._off_value = TRACK_STOP[self._rgb]
		...
		for index in range(4):
			self._grid[index + 4][7]._off_value = DEVICE_SELECT[self._rgb]
...
#		self._session2.set_track_bank_buttons(self._button[1], self._button[0])
		...
		self._send_reset.set_buttons(tuple(self._grid[index + 4][5] for index in range(4)))
		for index in range(4):
			self._grid[index][7]._off_value = DEVICE_SELECT[self._rgb]
	...
	def zoom_main(self):
		...
		for index in range(8):
			#self._grid[index][6]._on_value = CROSSFADE_TOGGLE[self._rgb]
			#self._mixer.channel_strip(index).set_crossfade_toggle(self._grid[index][6])
			self._grid[index][6]._on_value = TRACK_STOP[self._rgb]
			track_stop_buttons.append(self._grid[index][6])
		for index in range(5):
...
#		self._session_main.set_track_bank_buttons(self._button[5], self._button[4])
	...
	def r_function_update(self):
		...
		#self._session2.set_offsets(int(RIGHT_MODE_OFFSETS[mode]), self._session2._scene_offset)
	...
	def assign_channel_select_buttons(self):
		for index in range(4):
			#if(self._mixer.channel_strip(index)):
			self._grid[index][7]._off_value = 0
			self._grid[index][7]._on_value = 127
...
#		elif(self._r_function_mode._mode_index < 3):
	...
	def handle_sysex(self, midi_bytes):
		...
		if midi_bytes == tuple([240, 126, 0, 6, 2, 0, 1, 97, 1, 0, 7, 0, 15, 10, 0, 0, 247]):
			...
			self._rgb = 0
			self._host.surface = 'Ohm64_RGB'
			for index in range(128):
				self._host._monomap[index] = index
			#self._send_midi(tuple(assign_default_colors))
			...
			self._rgb = 1
			self._host.surface = 'Ohm64_MonoChrome'
			for index in range(1, 128):
				self._host._monomap[index] = (((index - 1)*7) + 1)  % 126
			self._assign_session_colors()
	...
	def disconnect(self):
		...
		self.log_message("--------------= MonOhm log closed =--------------") #Create entry in log file
