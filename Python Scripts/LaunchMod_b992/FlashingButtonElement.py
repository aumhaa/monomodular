...
#white cyan magenta red blue yellow green
#COLOR_MAP = [52, 55, 7, 21, 20, 5, 36]
#COLOR_MAP = [20, 36, 52, 21, 38, 55, 5, 6, 7]
COLOR_MAP = [20, 21, 5, 36, 38, 6, 52, 55, 7]
#COLOR_MAP = [1, 32, 2, 4, 8, 64, 16]
...
class FlashingButtonElement(ButtonElement):
	...
	def send_value(self, value, force_send = False):
		if(type(self) != type(None)):
			...
			if (force_send or ((value != self._last_sent_value) and self._is_being_forwarded)):
				...
				if value in range(1, 126):
					data_byte2 = COLOR_MAP[(value-1) % 9]
				elif value == 127:
					data_byte2 = 52
				else:
					data_byte2 = 4
				...
				self._flash_state = round((value-1)/9)
	...
	def flash(self, timer):
		if (self._is_being_forwarded and self._flash_state in range(1, 13) and (timer % self._flash_state) == 0):
			data_byte1 = self._original_identifier
