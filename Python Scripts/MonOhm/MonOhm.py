...
class MonOhm(ControlSurface):
	...
	def _assign_session_colors(self):
		num_tracks = 4
		num_scenes = 5 
		self._session.set_stop_clip_value(self._color_defs['STOP_ALL'])
		self._session2.set_stop_clip_value(self._color_defs['STOP_ALL'])
		self._session_main.set_stop_clip_value(self._color_defs['STOP_ALL'])
		for row in range(num_scenes): 
			for column in range(num_tracks):
