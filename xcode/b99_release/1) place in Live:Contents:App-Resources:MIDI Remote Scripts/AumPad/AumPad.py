...
#		elif(self._r_function_mode._mode_index < 3):
	...
	def disconnect(self):
		...
		self.log_message(time.strftime("%d.%m.%Y %H:%M:%S", time.localtime()) + "--------------= AumPad log closed =--------------") #Create entry in log file
	...
	def _set_brightness(self, value):
		...
		for control in self.controls:
			if isinstance(control, OSCFlashingButtonElement):
				self._monobridge._send_osc(control.osc_alt, int(self._bright))
