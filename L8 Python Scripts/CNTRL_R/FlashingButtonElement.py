...
from Cntrlr_Map import COLOR_MAP
...
class FlashingButtonElement(ButtonElement):
	...
	def turn_on(self, Force = False):
		self.send_value(self._on_value, self._last_sent_value==-1)

	def turn_off(self, Force = False):
		self.send_value(self._off_value, self._last_sent_value==-1)
	...
	def install_connections(self):	#this override has to be here so that translation will happen when buttons are disabled
		if not self._is_enabled and ((self._msg_channel != self._original_channel) or (self._msg_identifier != self._original_identifier)):
