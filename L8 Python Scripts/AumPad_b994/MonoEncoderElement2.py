...
class MonoEncoderElement2(EncoderElement):
	...
	def forward_parameter_value(self):
		if(not (type(self._parameter) is type(None))):
			...
			if(parameter!=self._parameter_last_value):
				...
				self._script._monobridge._send_osc(self.osc_parameter, self._script.generate_strip_string(self._parameter_last_value), True)
	...
	def forward_parameter_name(self):
		if(not (type(self._parameter) is type(None))):
			...
			if parameter:
				...
				self._script._monobridge._send_osc(self.osc_name, self._script.generate_strip_string(self._parameter_lcd_name))	
	...
	def add_parameter_listener(self, parameter):
		...
		if parameter:
			...
			try:
				...
				self._parameter_last_value = ' '
			self._script._monobridge._send_osc(self.osc_name, self._script.generate_strip_string(self._parameter_lcd_name))
			self._script._monobridge._send_osc(self.osc_parameter, self._script.generate_strip_string(self._parameter_last_value))
			new_value=float((self._parameter.value - self._parameter.min) / (self._parameter.max - self._parameter.min))
			self._script._monobridge._send_osc(self.osc, new_value)
	...
	def remove_parameter_listener(self, parameter):
		...
		if parameter:
			...
			self._script._monobridge._send_osc(self.osc_name, '`_')
			self._script._monobridge._send_osc(self.osc_parameter, '`_')
