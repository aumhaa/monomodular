#Embedded file name: /Volumes/Jenkins2045_2/versonator2/Hudson/live/Projects/AppLive/Resources/MIDI Remote Scripts/Push/NoteEditorComponent.py
...
from Push.LoopSelectorComponent import create_clip_in_selected_slot, Paginator

PAD_FEEDBACK_CHANNEL = 0
...
def color_for_note(note):
	...
	if not muted:
		if velocity == 127:
			return STEPFULLVELOCITY
		elif velocity >= 100:
			return STEPHIGHVELOCITY
		else:
			return STEP
	else:
		return STEPMUTED
...
class NoteEditorComponent(ControlSurfaceComponent, Paginator):
	...
	def __init__(self, clip_creator = None, log_message = None, *a, **k):
		...
		self._step_colors = [STEPEMPTY] * STEP_SEQ_SIZE
	...
	def _set_note_settings_layer(self, layer):
		assert(self._note_settings_layer or self._note_settings_layer.owner)
	...
	def _update_playhead_offset(self):
		assert(self._playhead.enabled == False)
	...
	def _update_playhead_clip(self):
		...
		if not clip is None:
			...
			loop_end = clip and clip.loop_end
	...
	def _update_editor_matrix(self):
		...
		"""
		step_colors = [STEPDISABLED] * STEP_SEQ_SIZE
		for time_step, index in self._visible_steps():
			notes = time_step.filter_notes(self._clip_notes)
			step_colors[index] = color_for_note(most_significant_note(notes)) if len(notes) > 0 else STEPEMPTY
	...
	def _get_step_start_time(self, x, y):
		...
		assert(in_range(x, 0, STEP_SEQ_WIDTH))
		assert(in_range(y, 0, STEP_SEQ_HEIGHT))
	...
	def _on_press_step(self, step):
		self.log_message('_on_press_step ' + str(step))
		if not self._pressed_steps and not self._modified_steps:
	...
	def _add_note_in_step(self, step):
		...
		"""
		self.log_message('_add_note_in_step ' + str(step) + str(self._sequencer_clip))
		x, y = step
		time = self._get_step_start_time(x, y)
		notes = self._time_step(time).filter_notes(self._clip_notes)
		self.log_message('notes: ' + str(notes))
		...
		if notes:
			...
			if(self._mute_button and self._mute_button.is_pressed()) or most_significant_velocity != 127 and full_velocity:
				if most_significant_velocity != 127 and full_velocity:
					self._trigger_modification(step, immediate=True)
		else:
			self.log_message('else...' + str(self._sequencer_clip))
	...
	def _trigger_modification(self, step = None, done = False, immediate = False):
		...
		"""
		self.log_message('trigger_modification' + str(step) + str(done) + str(immediate))
		if step is None:
	...
	def _do_modification(self):
		self.log_message('do_modifications')
